...
系统默认使用 **SQLite**，开箱即用，无需单独安装数据库——执行迁移即可自动创建数据库文件：

```bash
python manage.py migrate
```

> 💡 如需用于生产环境，可在 `inventory/settings.py` 的 `DATABASES` 中改用 PostgreSQL（项目已内置 `psycopg2` 驱动），再执行上述迁移。
