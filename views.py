...
def product_edit(request, product_id):
    ...
    if request.method == 'POST':
        print("POST 请求数据:", request.POST)  # 打印 POST 数据
        print("FILES 数据:", request.FILES)    # 打印文件数据
        print("Content-Type:", request.content_type)  # 打印 Content-Type
        print("请求头:", request.headers)  # 打印请求头
        
        # 检查是否有文件上传
        if 'image' in request.FILES:
            print("上传的文件:", request.FILES['image'])
            print("文件大小:", request.FILES['image'].size)
            print("文件名称:", request.FILES['image'].name)
            print("文件类型:", request.FILES['image'].content_type)
        else:
            print("没有找到image字段在FILES中")
            print("FILES中的字段:", list(request.FILES.keys()))
        ...
        if form.is_valid():
            ...
            try:
                product = form.save()
                print("表单保存成功")  # 打印保存状态
                print("图片路径:", product.image.path if product.image else "无图片")  # 打印图片路径
                print("图片URL:", product.image.url if product.image else "无URL")  # 打印图片URL
                ...
                print("保存失败:", str(e))  # 打印错误信息
                messages.error(request, f'保存失败: {str(e)}')
        else:
            print("表单验证失败:", form.errors)  # 打印验证错误
    else:
