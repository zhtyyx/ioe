...
def normalize_sale_payment_method(payment_method, default='cash'):
    """Normalize legacy payment names and reject unsupported methods."""
    normalized = payment_method or default
    if normalized == 'account':
        normalized = 'balance'

    valid_methods = {value for value, _label in Sale.PAYMENT_METHODS}
    if normalized not in valid_methods:
        raise ValueError(f'不支持的支付方式: {normalized}')
    return normalized
...
def sale_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                sale.payment_method = normalize_sale_payment_method(
                    request.POST.get('payment_method'), default='cash'
                )
        ...
        if form.is_valid():
            try:
                with transaction.atomic():
                    ...
                    payment_method = normalize_sale_payment_method(
                        request.POST.get('payment_method'), default=sale.payment_method
                    )
