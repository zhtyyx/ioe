...
def normalize_sale_payment_method(payment_method):
    """Normalize legacy aliases and reject payment modes the backend cannot settle."""
    normalized = payment_method or 'cash'
    if normalized == 'account':
        normalized = 'balance'

    allowed_payment_methods = {choice[0] for choice in Sale.PAYMENT_METHODS}
...
def sale_create(request):
    ...
    if request.method == 'POST':
        ...
        if total_amount <= 0 and valid_products_data:
            messages.error(request, '销售单金额无效，请核对商品价格后重新提交。')
            return redirect('sale_create')
