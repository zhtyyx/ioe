...
def _normalize_payment_method(payment_method):
    """Map POS UI aliases to persisted Sale payment method choices."""
    payment_method = payment_method or 'cash'
    aliases = {
        'account': 'balance',
        'credit': 'other',
    }
    normalized = aliases.get(payment_method, payment_method)
    valid_methods = {value for value, _ in Sale.PAYMENT_METHODS}
    return normalized if normalized in valid_methods else 'other'


def _apply_member_balance_payment(member, amount, sale, user):
    """Deduct member balance using a database-side increment after the caller locks the row."""
    ...
    member_service.apply_member_balance_change(member, -amount)
...
def sale_create(request):
    ...
    if request.method == 'POST':
        ...
        if total_amount <= 0 and valid_products_data:
            print("错误：计算的总金额仍然为0或负数，中止创建销售单")
            messages.error(request, '销售单金额计算失败，请检查商品价格后重试。')
            return redirect('sale_create')
        ...
        try:
            with transaction.atomic():
                ...
                if form.is_valid():
                    ...
                    # 设置支付方式
                    sale.payment_method = _normalize_payment_method(request.POST.get('payment_method'))
                    sale.balance_paid = Decimal('0.00')

                    # 如果使用余额支付，处理会员余额
                    if sale.payment_method == 'balance':
                        if not member:
                            raise ValueError('需要选择会员才能使用余额支付')
                        sale.balance_paid = _apply_member_balance_payment(
                            member,
                            sale.final_amount,
                            sale,
                    ...
                    # 如果是混合支付，处理余额部分
                        ...
                        if balance_amount > sale.final_amount:
                            raise ValueError('余额支付金额不能超过销售实付金额')
                        if balance_amount > 0:
                            sale.balance_paid = _apply_member_balance_payment(
                                member,
                                balance_amount,
