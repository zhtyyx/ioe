...
def normalize_sale_payment_method(payment_method):
    """Map POS UI aliases to the Sale model's persisted payment choices."""
    ...
    valid_methods = {value for value, _ in Sale.PAYMENT_METHODS}
    return payment_method if payment_method in valid_methods else 'other'


def deduct_member_balance_for_sale(member, amount, sale, operator):
    """Deduct member balance only if sufficient funds are still available."""
    if amount <= 0:
        return Decimal('0.00')
...
def update_member_purchase_stats(member_id, points_earned, final_amount):
    Member.objects.filter(pk=member_id).update(
        points=F('points') + points_earned,
        purchase_count=F('purchase_count') + 1,
        total_spend=F('total_spend') + final_amount,
        updated_at=timezone.now(),
    )
...
def sale_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # 设置支付方式
            sale.payment_method = normalize_sale_payment_method(request.POST.get('payment_method', 'cash'))
            ...
            try:
                with transaction.atomic():
                    ...
                    # 保存销售单基本信息。后续任何失败都会回滚，避免留下孤儿销售单。
                    sale.save()

                    if sale.payment_method == 'balance':
                        if not sale.member_id:
                            raise ValueError('余额支付需要关联会员')
                        sale.balance_paid = deduct_member_balance_for_sale(
                            sale.member,
                            sale.final_amount,
                            sale,
                            request.user,
                    ...
                    for item_data in valid_products_data:
                        inventory_obj = Inventory.objects.select_for_update().get(pk=item_data['inventory'].pk)
                        ...
                        # 更新库存
                        Inventory.objects.filter(pk=inventory_obj.pk).update(
        ...
        if form.is_valid():
            try:
                with transaction.atomic():
                    ...
                    # 设置支付方式
                    payment_method = normalize_sale_payment_method(request.POST.get('payment_method') or sale.payment_method)
                    ...
                    if payment_method == 'balance':
                        ...
                        if sale.balance_paid > 0:
                            raise ValueError('该销售单已扣过会员余额')
                        balance_amount = sale.final_amount
                    elif payment_method == 'mixed':
                        if not member:
                            raise ValueError('混合支付需要关联会员')
                        if sale.balance_paid > 0:
                            raise ValueError('该销售单已扣过会员余额')
                        try:
