...
SUPPORTED_SALE_CREATE_PAYMENT_METHODS = {'cash', 'wechat', 'alipay', 'card', 'balance', 'other'}
SUPPORTED_SALE_COMPLETE_PAYMENT_METHODS = {value for value, _label in Sale.PAYMENT_METHODS}


def normalize_payment_method(payment_method):
    """Normalize legacy frontend values before validating payment handling."""
...
def sale_detail(request, sale_id):
    """销售单详情视图"""
    sale = get_object_or_404(Sale.objects.select_related('member', 'operator'), pk=sale_id)
...
def sale_create(request):
    ...
    if request.method == 'POST':
        ...
        if total_amount <= 0 and valid_products_data:
            messages.error(request, '销售单创建失败，无法计算有效的销售金额。')
            return redirect('sale_create')
        ...
        if form.is_valid():
            ...
            payment_method = normalize_payment_method(request.POST.get('payment_method', 'cash'))
            if payment_method not in SUPPORTED_SALE_CREATE_PAYMENT_METHODS:
        ...
        if form.is_valid():
            try:
                with transaction.atomic():
                    ...
                    payment_method = normalize_payment_method(request.POST.get('payment_method') or sale.payment_method)
                    if payment_method not in SUPPORTED_SALE_COMPLETE_PAYMENT_METHODS:
