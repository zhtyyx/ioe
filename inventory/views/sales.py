...
def sale_create(request):
    ...
    if request.method == 'POST':
        ...
        if total_amount <= 0 and valid_products_data:
            messages.error(request, '销售单创建失败，商品金额必须大于0。')
            return redirect('sale_create')
        ...
        if form.is_valid():
            ...
            allowed_payment_methods = {'cash', 'wechat', 'alipay', 'card', 'balance', 'other'}
            if payment_method not in allowed_payment_methods:
                messages.error(request, '销售单创建失败，不支持的支付方式。')
                return redirect('sale_create')
        ...
        if form.is_valid():
            try:
                with transaction.atomic():
                    ...
                    if payment_method == 'account':
                        payment_method = 'balance'
                    if payment_method not in {value for value, _ in Sale.PAYMENT_METHODS}:
...
def sale_delete_item(request, sale_id, item_id):
    ...
    try:
        ...
        messages.error(request, '找不到商品库存记录，未删除销售明细')
