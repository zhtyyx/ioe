...
def _validated_payment_method(payment_method, *, allow_mixed=True):
    """Normalize legacy payment values and reject unsupported settlement paths."""
    payment_method = payment_method or 'cash'
    if payment_method == 'account':
        payment_method = 'balance'

    valid_methods = {method for method, _label in Sale.PAYMENT_METHODS}
    if payment_method not in valid_methods:
        raise ValueError('不支持的支付方式')
    if payment_method == 'mixed' and not allow_mixed:
        raise ValueError('当前收银台暂不支持混合支付')
...
def sale_create(request):
    ...
    if request.method == 'POST':
        ...
        if total_amount <= 0 and valid_products_data:
            messages.error(request, '销售单创建失败，商品总金额无效。')
            return redirect('sale_create')
        ...
        if form.is_valid():
            ...
            try:
                sale.payment_method = _validated_payment_method(
        ...
        if form.is_valid():
            try:
                with transaction.atomic():
                    ...
                    payment_method = _validated_payment_method(
