...
SUPPORTED_SALE_COMPLETE_PAYMENT_METHODS = {
    value for value, _label in Sale.PAYMENT_METHODS
}


def _normalize_payment_method(raw_payment_method, supported_methods):
    """Map legacy values and reject unimplemented payment paths before persistence."""
    payment_method = raw_payment_method or 'cash'
...
def sale_create(request):
    ...
    if request.method == 'POST':
        ...
        if total_amount <= 0 and valid_products_data:
            messages.error(request, '销售单创建失败，销售金额无效。')
            return redirect('sale_create')
        ...
        if form.is_valid():
            ...
            try:
                sale.payment_method = _normalize_payment_method(
                    request.POST.get('payment_method', 'cash'),
                    SUPPORTED_SALE_CREATE_PAYMENT_METHODS,
                )
...
def sale_delete_item(request, sale_id, item_id):
    """删除销售单商品视图"""
    if request.method != 'POST':
        messages.error(request, '删除销售商品需要提交确认。')
