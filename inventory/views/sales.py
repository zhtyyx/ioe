...
SUPPORTED_PAYMENT_METHODS = {value for value, _label in Sale.PAYMENT_METHODS}


def normalize_payment_method(payment_method):
    """Normalize legacy payment values and reject flows the system cannot settle."""
...
def sale_create(request):
    ...
    if request.method == 'POST':
        ...
        # 最终安全检查：金额无法计算时不能用固定值完成销售。
        if (total_amount <= 0 or final_amount <= 0) and valid_products_data:
            ...
            try:
                with transaction.atomic():
                    ...
                    if sale.final_amount <= 0:
                        raise ValueError('销售单金额无效，不能完成收款')
