...
class SaleStatusTest(TestCase):
    ...
    def test_get_delete_item_is_rejected_without_restoring_inventory(self):
        sale = self._make_sale(status='DRAFT')
        item = sale.items.get()
        self.inventory.refresh_from_db()
        before = self.inventory.quantity
    ...
    def test_sale_detail_does_not_recalculate_persisted_amounts(self):
        sale = self._make_sale(status='COMPLETED')
        ...
        self.assertEqual(sale.final_amount, Decimal('95.00'))
