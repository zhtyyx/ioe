...
class SaleStatusTest(TestCase):
    ...
    def test_cannot_delete_item_from_completed_sale(self):
        sale = self._make_sale(status='COMPLETED')
        item = sale.items.get()
        self.inventory.refresh_from_db()
        before = self.inventory.quantity
    ...
    def test_sale_detail_does_not_rewrite_historical_amounts(self):
        sale = self._make_sale(status='COMPLETED')
        sale.total_amount = Decimal('15.00')
        sale.discount_amount = Decimal('0.00')
        sale.final_amount = Decimal('15.00')
        sale.save()
