...
class LogFileViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.log_path = os.path.join(settings.LOG_DIR, 'test-admin.log')
        with open(self.log_path, 'w', encoding='utf-8') as log_file:
            log_file.write('important audit line\n')
        self.addCleanup(lambda: os.path.exists(self.log_path) and os.remove(self.log_path))

    def test_download_log_file_records_nullable_content_type_audit(self):
        response = self.client.get(reverse('download_log_file', args=['test-admin.log']))

        self.assertEqual(response.status_code, 200)
        log_entry = LogEntry.objects.get(object_id='test-admin.log', action_flag=1)
        self.assertIsNone(log_entry.content_type_id)

    def test_delete_log_file_records_nullable_content_type_audit(self):
        response = self.client.post(
            reverse('delete_log_file', args=['test-admin.log']),
            {'confirm': 'on'},
        )

        self.assertRedirects(response, reverse('log_list'))
        self.assertFalse(os.path.exists(self.log_path))
        log_entry = LogEntry.objects.get(object_id='test-admin.log', action_flag=3)
        self.assertIsNone(log_entry.content_type_id)
